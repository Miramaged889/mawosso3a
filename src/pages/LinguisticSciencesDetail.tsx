...
const LinguisticSciencesDetail: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const numericId = id ? parseInt(id) : null;

  const { data: item, error } = useEntry(numericId);
  const { data: relatedData } = useEntries({ category: CATEGORY_IDS.LINGUISTIC_SCIENCES });

  // Debug image URLs
  React.useEffect(() => {
    if (item) {
      console.log("Linguistic Sciences item cover_image:", item.cover_image);
      console.log(
        "Linguistic Sciences item full URL:",
        item.cover_image
  ...
  const breadcrumbItems = [
    { label: "العلوم اللغوية", path: "/linguistic-sciences" },
    { label: item.title }
  ];
...
export default LinguisticSciencesDetail; 